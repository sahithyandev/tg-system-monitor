...
if command -v systemctl >/dev/null 2>&1; then
    ...
    if systemctl is-active --quiet tgsm; then
        ...
        echo "║       ${CONFIG_FILE}"
        echo "║                                                              ║"
        echo "║     • Set bot_token to enable the Telegram bot and alerts.   ║"
        echo "║       Without it, the monitor runs in collector-only mode.   ║"
        echo "║                                                              ║"
        echo "║  2. Set the join password (only needed with Telegram bot):   ║"
